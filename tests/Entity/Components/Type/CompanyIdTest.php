...
class CompanyIdTest extends TestCase
{
    ...
    public function testSerializeVersion10200(): void
    {
        ...
        $this->assertEquals('CompanyID', $xml->getName());
        $this->assertEquals($companyId, strval($xml));
        $this->assertEquals(ElectronicAddressScheme::NLCommerceNumber->getSchemeId(Versions::VERSION_1_2), $xml['schemeID']);
        $this->assertEquals(ElectronicAddressScheme::NLCommerceNumber->getSchemeAgencyId(Versions::VERSION_1_2), $xml['schemeAgencyID']);
        $this->assertContainsEquals(
            'urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2',
            $xml->getDocNamespaces()
        );
