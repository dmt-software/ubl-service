...
namespace DMT\Ubl\Service\Visitor;
...
final class XmlSerializationVisitor extends AbstractVisitor implements SerializationVisitorInterface
{
    ...
    public function visitProperty(PropertyMetadata $metadata, $v): void
    {
        ...
        if ($addEnclosingElement) {
            ...
            if ($v instanceof \DMT\Ubl\Service\Entity\Invoice\Type\AllowanceTotalAmount) {
                var_dump($v, empty($v), $this->isElementEmpty($element));
            }

            if ($this->isElementEmpty($element) && (empty($v) || $this->isSkippableCollection($metadata) || $this->isSkippableEmptyObject($node, $metadata))) {
    ...
    private function isElementEmpty(\DOMElement $element): bool
    {
        ...
        return !$element->hasChildNodes() || trim($element->textContent) === '';
    }
